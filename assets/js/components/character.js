Vue.component('character', {
  ...
  created: function () {
    ...
    let npc_list = config.db.map.zone1.layerNpc
  ...
  methods: {
    moveAction: function (e) {
        ...
        let cell_tiles = config.db.map.zone1.map
    ...
    changedCharacterPostiton: function (previous_pos) {
      ...
      if (!config.db.map.zone1.layerNpc[position[1]]) {
        Vue.set(config.db.map.zone1.layerNpc, position[1], {})  
      }
      if (!config.db.map.zone1.layerNpc[position[1]][position[0]]) {
        Vue.set(config.db.map.zone1.layerNpc[position[1]], position[0], {})  
      }

      //console.log(previous_pos, position)
      if (config.db.map.zone1.layerNpc[previous_pos[1]] && config.db.map.zone1.layerNpc[previous_pos[1]][previous_pos[0]]) {
        Vue.delete(config.db.map.zone1.layerNpc[previous_pos[1]][previous_pos[0]], config.character.id)  
      }

      Vue.set(config.db.map.zone1.layerNpc[position[1]][position[0]], config.character.id, {
              id: "7",
              name: "Player Kokoko",
              type: "player",
      })
